...
pub fn decrypt(
    ...
    if (tag.len != 16) return Error.DecryptionFailed;
    if (out_plaintext.len != ciphertext.len) return Error.DecryptionFailed;
    var key_array: [32]u8 = undefined;
    ...
    crypto.aead.chacha_poly.ChaCha12Poly1305.decrypt(out_plaintext, ciphertext, tag_array, "", nonce_array, key_array) catch {
        return Error.DecryptionFailed;
    };
}
